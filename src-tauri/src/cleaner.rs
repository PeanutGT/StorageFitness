    ...
    fn perform_recycle(paths: Vec<String>) -> EngineResult<()> {
        unsafe {
            ...
            let file_op: IFileOperation = CoCreateInstance(&FileOperation, None, CLSCTX_ALL)
                .map_err(|e| EngineError::Win32Error { operation: "CoCreateInstance(IFileOperation)".to_string(), message: e.to_string() })?;
            ...
            file_op.SetOperationFlags(op_flags)
                .map_err(|e| EngineError::Win32Error { operation: "IFileOperation::SetOperationFlags".to_string(), message: e.to_string() })?;
                ...
                match item_result {
                    Ok(item) => {
                        ...
                        if let Err(e) = file_op.DeleteItem(&item, None) {
                            eprintln!("Warning: Failed to add item to delete queue: {} - {}", path_str, e);
                        }
                    }
                    Err(e) => {
                        // 若檔案已不存在等情況，記錄並忽略，繼續處理下一個
                        eprintln!("Warning: Failed to create IShellItem for {}: {}", path_str, e);
                    }
                }
            }
            ...
            file_op.PerformOperations()
                .map_err(|e| EngineError::Win32Error { operation: "IFileOperation::PerformOperations".to_string(), message: e.to_string() })?;
